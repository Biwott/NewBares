...
class AllReferals extends Component
{
    ...
    public $page_title = "List of All Referals";
    public $search = '';
    ...
    public function render()
    {
        return view('livewire.admin.affiliates.all', [
            ...
            User::where('username', $this->search)->exists()
                ? User::where('referer_id', User::where('username', $this->search))->latest()->paginate(10)
                : ($this->search != ''
                    ? User::where('referer_id', 99999999999)->latest()->paginate(10)
                    : User::where('referer_id', '!=', 0)->latest()->paginate(10))
