...
class ClosedTrades extends Component
{
    ...
    {
        return view(
            'livewire.admin.forex.closed-trades',
            [
                'trades' => User::where('username', $this->search)->exists()
                    ?  Trade::where('user_id', User::where('username', $this->search)->first()->id)->where('status', 3)->orderBy('opened_at', 'DESC')->paginate(15)
                    : ($this->search != ''
                        ? Trade::where('id', 0)->paginate(15)
                        : Trade::where('status', 3)->orderBy('opened_at', 'DESC')->paginate(15))
            ]
