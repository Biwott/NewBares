...
class DeclinedTradedrawals extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        return view('livewire.admin.forex.tradedrawals', [
            ...
            User::where('username', $this->search)->exists()
                ?  Tradedrawal::where('status', 2)->where('user_id', User::where('username', $this->search)->first()->id)->orderBy('created_at', 'DESC')->paginate(10)
                : ($this->search != ''
                    ? Tradedrawal::where('id', 0)->paginate(10)
                    : Tradedrawal::where('status', 2)->orderBy('created_at', 'DESC')->paginate(10))
