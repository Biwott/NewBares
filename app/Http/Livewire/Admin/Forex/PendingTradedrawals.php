...
class PendingTradedrawals extends Component
{
    use WithPagination;
    ...
    public function approve(Tradedrawal $tradedrawal)
    {
        ...
        return redirect()->route('admin.forex.pending-tradedrawals');
    }
    ...
    {
        return view('livewire.admin.forex.pending-tradedrawals', [
            'tradedrawals' =>
            User::where('username', $this->search)->exists()
                ?  Tradedrawal::where('status', 0)->where('user_id', User::where('username', $this->search)->first()->id)->orderBy('created_at', 'DESC')->paginate(10)
                : ($this->search != ''
                    ? Tradedrawal::where('id', 0)->paginate(10)
                    : Tradedrawal::where('status', 0)->orderBy('created_at', 'DESC')->paginate(10))
